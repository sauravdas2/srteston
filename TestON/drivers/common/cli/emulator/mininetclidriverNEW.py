...
class MininetCliDriver(Emulator):
    ...
    def fping_loss(self,**pingParams):
        ''' 
        Detailed fping usage for measuring packet loss
        '''
        main.log.info("Using fping to measure packet loss...please wait") 
        args = utilities.parse_args(["SRC","TARGET","SIZE"],**pingParams)
        command = args["SRC"] + " fping -s -b "+args["SIZE"]+" -c 10 "+args["TARGET"]
        self.handle.sendline(command) 
        self.handle.expect([args["TARGET"],pexpect.EOF,pexpect.TIMEOUT]) 
        self.handle.expect(["mininet",pexpect.EOF,pexpect.TIMEOUT])
        response = self.handle.before 
